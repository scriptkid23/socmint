...
/** Radix/React useId-style ids change every mount. */
export const UNSTABLE_DOM_ID_SELECTOR =
  /^#(?:radix-|:r[0-9a-z]+:|[\w-]+-_r_[a-z0-9]+-)/i;

const SUFFIX_TRIGGER_SELECTOR = /\[id\$="-trigger-[^"]+"\]/i;

/** Extract stable Radix trigger tail from ids like `radix-:r_b:-trigger-nft-details`. */
export function radixTriggerSuffix(id: string): string | null {
  const match = id.trim().match(/-trigger-(.+)$/i);
  return match ? match[1] : null;
}

/** Build a session-stable CSS selector for Radix tab/trigger elements. */
export function stableRadixTriggerSelector(
  tag: string,
  suffix: string,
  role?: string | null,
): string {
...
export function replayClickSelector(step: Extract<RecordedStep, { type: 'click' }>): string {
